...
if(params$bank=="Ger") {
  ...
  dashed <- "The dashed blue and red lines are the respective long term medians."
}

if(!params$bank=="Ger") {
  ltm_ts <- "The dashed line is the long term median."
  dashed <- "The dashed blue line is the long term median."
}
...
highlights$thisyear[highlights$variable=="sh_for_mcreg" & highlights$bank==params$bank] <- as.numeric(highlights$thisyear[highlights$variable=="sh_for_mcreg" & highlights$bank==params$bank]) %>%
  formatC(zero.print = "") %>%
  as.character()
```
...
```{r, echo=F, message=F, out.width="100%", fig.align="center", fig.cap=paste0("Survey station map. There were ", sum(ntows$Freq[ntows$bank==params$bank]), " tow locations on ", bankname, " in ", year, "."), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/survey_strata.png"))
```
...
```{r, echo=FALSE, message=F, out.height="85%", fig.align="center", fig.cap=paste0("Biomass time series for ", bankname, " in ", year, ", represented by kilograms (kg) of scallop per standardized tow (y-axis). Pre-recruits are shown in the top panel, recruit-sized scallop in the middle panel, and fully-recruited scallop are in the bottom panel. ", ltm_ts), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/biomass_ts.png"))
```
...
```{r, echo=FALSE, message=F, out.height="80%", fig.align="center", fig.cap=paste0("Shell height frequencies of scallop on ", bankname, " in ", year, ". Vertical lines are the recruit size limits."), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/SHF.png"))
```
...
```{r, echo=FALSE, message=F, out.height="70%", fig.align="center", fig.cap=paste0("Meat weights and shell heights for sampled tows on ", bankname, " in ", year, " (top). Red points represent individual scallops, red lines represent the trend for each sampled tow, and the solid blue line is the overall trend for the current survey. Condition factor (bottom) is measured as grams of meat per decimeter cubed, which can be thought of as the meat weight of a scallop with a shell height of 100mm. ", dashed), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/MWSH_and_CF_ts.png"))
```
...
Patterns in natural mortality are represented by the percentage of clappers (dead scallop, with empty hinged shells) caught in the survey tows. The percentage is calculated as the number of clappers in each size class relative to the total of dead and live scallop caught in that size class (Figure `r fignum[6]`). Survey tows are standardized to 800 metres. `r if(params$bank %in% c("Ban", "BanIce", "Mid", "GB")) fig1foot` Table `r tabnum[3]` provides the values for the current and previous surveys, and long term medians, by size class.
...
```{r, echo=FALSE, message=F, out.height="85%", fig.align="center", fig.cap=paste0("Clapper time series for ", bankname, " in ", year, ", represented by percentage of clappers per tow (y-axis). Pre-recruits are shown in the top panel, recruit-sized scallop in the middle panel, and fully-recruited scallop are in the bottom panel. ", ltm_ts), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/Clapper_per_ts.png"))
```
...
```{r, echo=FALSE, message=F, out.width="100%", fig.align="center", fig.cap=paste0("Spatial distribution of pre-recruit scallop on ", bankname, " in ", year, ". Each point is a survey station, and the colour shows the distribution of modelled abundance."), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/PR-spatial.png"))
```

[Return to bank summary](#`r paste0(params$bank, "links")`)
\newpage

```{r, echo=FALSE, message=F, out.width="100%", fig.align="center", fig.cap=paste0("Spatial distribution of recruit scallop on ", bankname, " in ", year, ". Each point is a survey station, and the colour shows the distribution of modelled abundance."), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/Rec-spatial.png"))
```

[Return to bank summary](#`r paste0(params$bank, "links")`)
\newpage

```{r, echo=FALSE, message=F, out.width="100%", fig.align="center", fig.cap=paste0("Spatial distribution of fully-recruited scallop on ", bankname, " in ", year, ". Each point is a survey station, and the colour shows the distribution of modelled abundance."), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/FR-spatial.png"))
```
...
The spatial distribution of condition factor is shown in Figure `r fignum[10]`, and the spatial distribution of meat count is shown in Figure `r fignum[11]`. Statistical summaries of condition factor and meat count are provided in Table `r tabnum[5]`. Maps are based on a geostatistical model that uses a Gaussian Markov Random Field (GMRF) to predict the spatial distribution. These models may not represent the full range of values observed during the survey.
...
```{r, echo=FALSE, message=F, out.width="100%", fig.align="center", fig.cap=paste0("Spatial distribution of scallop condition on ", bankname, " in ", year, "."), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/CF-spatial.png"))
```

[Return to bank summary](#`r paste0(params$bank, "links")`)
\newpage

```{r, echo=FALSE, message=F, out.width="100%", fig.align="center", fig.cap=paste0("Spatial distribution of meat count on ", bankname, " in ", year, "."), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/MC-spatial.png"))
```
...
The spatial distribution of clappers is shown in Figure `r fignum[12]`. Maps are based on a geostatistical model that uses a Gaussian Markov Random Field (GMRF) to predict the spatial distribution. The model may not represent the full range of values observed during the survey.

```{r, echo=FALSE, message=F, out.width="100%", fig.align="center", fig.cap=paste0("Spatial distribution of clappers on ", bankname, " in ", year, "."), warning=F}
knitr::include_graphics(paste0("C:/Users/keyserf/Documents/temp_data/", year, "/Presentations/Survey_summary/", params$bank, "/Clap-spatial.png"))
```
